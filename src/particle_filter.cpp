...
void ParticleFilter::init(double x, double y, double theta, double std[]) {
  ...
  std::normal_distribution<double> Nx(x, std[0]);
  std::normal_distribution<double> Ny(y, std[1]);
  std::normal_distribution<double> Ntheta(theta, std[2]);
...
void ParticleFilter::prediction(double delta_t, double std_pos[], double velocity, double yaw_rate) {
  ...
  for(int i = 0; i < num_particles; i++){
    ...
    normal_distribution<double> Ny(new_y, std_pos[1]);
    normal_distribution<double> Ntheta(new_theta, std_pos[2]);
